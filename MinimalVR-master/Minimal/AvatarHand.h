// This renders avatar hands, hopefully...
...
public:
	...
	************************************************************************************/
	...
	GLuint _compileProgramFromSource(const char vertexShaderSource[], const char fragmentShaderSource[], size_t errorBufferSize, char* errorBuffer) {
	...
	GLuint _compileProgramFromFiles(const char vertexShaderPath[], const char fragmentShaderPath[], size_t errorBufferSize, char* errorBuffer) {
	...
	************************************************************************************/
	...
	glm::mat4 _computeReflectionMatrix(const glm::vec4& plane)
	{
		return glm::mat4(
	...
	************************************************************************************/
	...
	void _destroyOVR(ovrSession session)
	{
		if (session)
		{
			ovr_Destroy(session);
	...
	************************************************************************************/
	...
	AvatarHand() {
		_combineMeshes = true;
		_avatarCombinedMeshAlpha = 0;
		_waitingOnCombinedMesh = false;

		_skinnedMeshProgram = LoadShaders("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader.glsl");
		_skinnedMeshPBSProgram = LoadShaders("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShaderPBS.glsl");
		_combinedMeshProgram = LoadShaders("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader_CombinedMesh.glsl");
	}
	...
	//	_skinnedMeshProgram = LoadShaders("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader.glsl");//_compileProgramFromFiles("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader.glsl", sizeof(errorBuffer), errorBuffer);

	//	_skinnedMeshPBSProgram = _compileProgramFromFiles("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShaderPBS.glsl", sizeof(errorBuffer), errorBuffer);

	//	_combinedMeshProgram = _compileProgramFromFiles("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader_CombinedMesh.glsl", sizeof(errorBuffer), errorBuffer);
	...
	//	return 0;
	//}
	//int initialization(ovrSession ovr_){
	...
	//	window = SDL_CreateWindow("Mirror", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, MIRROR_WINDOW_WIDTH, MIRROR_WINDOW_HEIGHT, SDL_WINDOW_OPENGL);
	//	if (!window)
	//	{
	...
	//	// Compile the reference shaders
	//	char errorBuffer[512];
	//	_skinnedMeshProgram = _compileProgramFromFiles("\shaders\\AvatarVertexShader.glsl", "\shaders\\AvatarFragmentShader.glsl", sizeof(errorBuffer), errorBuffer);
	...
	//		hmdDesc = ovr_GetHmdDesc(ovr_);
	//		for (int eye = 0; eye < 2; ++eye)
	//		{
	//			eyeSizes[eye] = ovr_GetFovTextureSize(ovr_, (ovrEyeType)eye, hmdDesc.DefaultEyeFov[eye], 1.0f);
	...
	//			ovr_CreateTextureSwapChainGL(ovr_, &desc, &eyeSwapChains[eye]);

	//			int length = 0;
	//			ovr_GetTextureSwapChainLength(ovr_, eyeSwapChains[eye], &length);
	//			for (int i = 0; i < length; ++i)
	//			{
	//				GLuint chainTexId;
	//				ovr_GetTextureSwapChainBufferGL(ovr_, eyeSwapChains[eye], i, &chainTexId);
	...
	//		ovr_CreateMirrorTextureGL(ovr_, &mirrorDesc, &mirrorTexture);

	//		GLuint mirrorTextureID;
	//		ovr_GetMirrorTextureBufferGL(ovr_, mirrorTexture, &mirrorTextureID);
	...
	//	if (ovr_)
	//	{
	//		ovr_SetTrackingOriginType(ovr_, ovrTrackingOrigin_FloorLevel);
	//		ovr_RecenterTrackingOrigin(ovr_);
	//	}
	...
	int renderHands()
	{
		...
		while (running) {
			...
			if (ovr)
			{
				...
				//-------
				/*ovrEyeRenderDesc& erd0 = eyeRenderDesc[0];
				ovrEyeRenderDesc& erd1 = eyeRenderDesc[1];
				ovrViewScaleDesc _viewScaleDesc0;
				_viewScaleDesc0.HmdSpaceToWorldScaleInMeters = 1.0f;
				_viewScaleDesc0.HmdToEyePose[0] = erd0.HmdToEyePose;
				ovrViewScaleDesc _viewScaleDesc1;
				_viewScaleDesc1.HmdSpaceToWorldScaleInMeters = 1.0f;
				_viewScaleDesc1.HmdToEyePose[1] = erd1.HmdToEyePose;
				ovr_GetEyePoses(ovr, frameIndex, ovrTrue, _viewScaleDesc0.HmdToEyePose, eyeRenderPose, &sensorSampleTime);*/
				...
				for (int eye = 0; eye < 2; ++eye)
				{
					...
					ovrEyeRenderDesc& erd = eyeRenderDesc[eye];
					ovrViewScaleDesc _viewScaleDesc;
					_viewScaleDesc.HmdSpaceToWorldScaleInMeters = 1.0f;
					_viewScaleDesc.HmdToEyePose[eye] = erd.HmdToEyePose;
					ovr_GetEyePoses(ovr, frameIndex, ovrTrue, _viewScaleDesc.HmdToEyePose, eyeRenderPose, &sensorSampleTime);
