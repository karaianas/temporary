...
public:
	...
	void renderHands(glm::mat4 view, glm::mat4 proj, glm::vec3 eyePos, ovrTrackingState trackState, ovrInputState inputState)
	{
		...
		if (_avatar)
		{
			...
			glm::vec3 hmdP = _glmFromOvrVector(trackState.HeadPose.ThePose.Position);
			glm::quat hmdQ = _glmFromOvrQuat(trackState.HeadPose.ThePose.Orientation);
			glm::vec3 leftP = _glmFromOvrVector(trackState.HandPoses[ovrHand_Left].ThePose.Position);
			glm::quat leftQ = _glmFromOvrQuat(trackState.HandPoses[ovrHand_Left].ThePose.Orientation);
			glm::vec3 rightP = _glmFromOvrVector(trackState.HandPoses[ovrHand_Right].ThePose.Position);
			glm::quat rightQ = _glmFromOvrQuat(trackState.HandPoses[ovrHand_Right].ThePose.Orientation);
			...
			_ovrAvatarHandInputStateFromOvr(left, inputState, ovrHand_Left, &inputStateLeft);

			ovrAvatarHandInputState inputStateRight;
			_ovrAvatarHandInputStateFromOvr(right, inputState, ovrHand_Right, &inputStateRight);

			//_updateAvatar(_avatar, deltaSeconds, hmd, inputStateLeft, inputStateRight, mic, playbackPacket, &playbackTime);
			_updateAvatarDependent(_avatar, 0.01f, hmd, inputStateLeft, inputStateRight);
		}
